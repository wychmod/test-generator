---
name: testcase-generator-gemini-adapter
description: 适配 Gemini CLI 的入口文件。Use when the host is Gemini CLI and expects a SKILL.md-style entry under its skills directory, and needs an entry for test case generation, requirement review, code-assisted testing, or structured testcase outputs.
---

# Gemini CLI Adapter for Testcase Generator

这是 `testcase-generator` 在 **Gemini CLI** 中的适配入口。

## 适配依据

Gemini CLI 官方技能目录为项目级 `.gemini/skills/<name>/SKILL.md`、用户级 `~/.gemini/skills/<name>/SKILL.md`，因此本技能**无需格式转换**即可被发现 ——
...
- Gemini CLI 上下文窗口较大，可在完整复杂任务中一次性读取 `prompts/phase0` ~ `phase5`。
...
Gemini CLI **不保证** Python 运行时可用。若脚本无法执行：
