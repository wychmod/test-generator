---
name: testcase-generator-kiro-adapter
description: 适配 Kiro 的入口文件。Use when the host is Kiro and expects a SKILL.md-style entry under its skills directory, and needs an entry for test case generation, requirement review, code-assisted testing, or structured testcase outputs.
---

# Kiro Adapter for Testcase Generator

这是 `testcase-generator` 在 **Kiro** 中的适配入口。

## 适配依据

Kiro 官方文档（kiro.dev/docs/skills）定义的技能目录为项目级 `.kiro/skills/<name>/SKILL.md`、用户级 `~/.kiro/skills/<name>/SKILL.md`，因此本技能**无需格式转换**即可被发现 ——
...
- Kiro 以 spec 驱动开发见长，可把本技能的 Phase 1/3 产物直接作为其 spec 的输入与校验依据。
