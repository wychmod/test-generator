---
name: testcase-generator-roo-adapter
description: 适配 Roo Code 的入口文件。Use when the host is Roo Code and expects a SKILL.md-style entry under its skills directory, and needs an entry for test case generation, requirement review, code-assisted testing, or structured testcase outputs.
---

# Roo Code Adapter for Testcase Generator

这是 `testcase-generator` 在 **Roo Code** 中的适配入口。

## 适配依据

Roo Code 官方文档（docs.roocode.com）定义的技能目录为项目级 `.roo/skills/<name>/SKILL.md`、用户级 `~/.roo/skills/<name>/SKILL.md`，因此本技能**无需格式转换**即可被发现 ——
...
- Roo Code 支持自定义模式与角色，建议为其配置「测试设计」模式并绑定本技能。
...
Roo Code **不保证** Python 运行时可用。若脚本无法执行：
