---
name: testcase-generator-githubcopilot-adapter
description: 适配 GitHub Copilot 的入口文件。Use when the host is GitHub Copilot and expects a SKILL.md-style entry under its skills directory, and needs an entry for test case generation, requirement review, code-assisted testing, or structured testcase outputs.
---

# GitHub Copilot Adapter for Testcase Generator

这是 `testcase-generator` 在 **GitHub Copilot** 中的适配入口。

## 适配依据

GitHub Copilot 官方技能目录为项目级 `.github/skills/<name>/SKILL.md`、用户级 `~/.copilot/skills/<name>/SKILL.md`，因此本技能**无需格式转换**即可被发现 ——
...
- 在 GitHub 场景下建议把测试设计产物落成仓库内 Markdown（便于随 PR 评审），而不是只在会话里输出。
...
GitHub Copilot **不保证** Python 运行时可用。若脚本无法执行：
