---
name: testcase-generator-clawdbot-adapter
description: 适配 Clawdbot 的入口文件。Use when the host is Clawdbot and expects a SKILL.md-style entry under its skills directory, and needs an entry for test case generation, requirement review, code-assisted testing, or structured testcase outputs.
---

# Clawdbot Adapter for Testcase Generator

这是 `testcase-generator` 在 **Clawdbot** 中的适配入口。

## 适配依据

Clawdbot 在仓库根使用 `skills/<name>/SKILL.md`，用户级为 `~/.clawdbot/skills/<name>/SKILL.md`，因此本技能**无需格式转换**即可被发现 ——
...
- 因本地技能目录位于仓库根 `skills/`，激活时注意不要与本仓库的 canonical 技能树混淆。
...
Clawdbot **不保证** Python 运行时可用。若脚本无法执行：
