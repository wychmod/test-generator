---
name: testcase-generator-pi-adapter
description: 适配 Pi 的入口文件。Use when the host is Pi and expects a SKILL.md-style entry under its skills directory, and needs an entry for test case generation, requirement review, code-assisted testing, or structured testcase outputs.
---

# Pi Adapter for Testcase Generator

这是 `testcase-generator` 在 **Pi** 中的适配入口。

## 适配依据

Pi 官方技能目录为项目级 `.pi/skills/<name>/SKILL.md`、用户级 `~/.pi/agent/skills/<name>/SKILL.md`，因此本技能**无需格式转换**即可被发现 ——
