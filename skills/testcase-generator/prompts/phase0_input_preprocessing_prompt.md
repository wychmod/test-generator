...
当输入中包含 `[参考知识]` 块时，必须把它识别为知识库检索结果，而不是用户当前需求本身。

处理规则：

- 保留每个知识片段的来源标识，格式应为 `KB: <filename>#<heading>`。
- 在 `00_input_validation_report.md` 中记录是否存在知识库上下文、命中来源和用途。
- 在 `00_normalized_input.md` 中单独保留 `[参考知识]` 区块，供 Phase 1/5 使用。
- 知识库不能覆盖用户当前需求；如果 `[参考知识]` 与用户当前输入冲突，标注 `KB 冲突`，并把当前输入作为主来源。
- 不要把 `[参考知识]` 中的历史用例直接改写成新需求，只能作为术语、规范、边界候选或历史参考。
...
| 文件名 | 内容 | 用途 |
|-------|------|------|
| `00_input_validation_report.md` | 输入验证报告（质量评分 + 类型 + 元信息） | 全局参考 |
| `00_normalized_input.md` | 规范化后的输入内容（传递给 P1） | P1 的正式输入 |
| `00_enhancement_suggestions.md` | 补充信息建议、歧义点、缺失项 | 用户补齐输入 |
| `00_blockers_and_assumptions.md` | 阻断项、强假设项、继续生成的前提条件 | 降级判定依据 |
...
> 敏感信息已被脱敏，原始信息参见 `00_input_validation_report.md`
...
> 基于 Phase 0 质量评估结果，以下是可以提升测试生成质量的补充信息。

## 🔴 必须补充（影响核心功能测试覆盖）

| # | 缺失项 | 影响 | 建议补充内容 |
|---|--------|------|-------------|
| 1 | [缺失项名称] | [对测试覆盖的具体影响] | [建议用户提供什么] |
...
| # | 歧义位置 | 现有表述 | 可能理解 A | 可能理解 B | 建议澄清方式 |
|---|---------|---------|-----------|-----------|-------------|
| 1 | [章节/行号] | [原文] | [理解 A] | [理解 B] | [向用户提什么问题] |
```
...
blocker_count: {N}
assumption_count: {N}
delivery_level: draft / degraded / formal
status: draft
version: 1.0
---
...
| 阻断ID | 类型 | 描述 | 影响的阶段/产物 | 是否可绕过 | 绕过代价 |
|--------|------|------|---------------|-----------|---------|
| BLK-001 | [乱码/字段缺失/状态机不闭合/规则冲突/验收标准不可判定] | [描述] | [Phase N / 产物名] | Y/N | [降级后果] |

> 若阻断项会直接影响正式测试用例的正确性，**不得**跳过说明后直接输出"正式交付物"，
> 只能降级为草稿、风险清单或带假设的结果。

## 2. 强假设项（Assumptions）

| 假设ID | 假设内容 | 依据 | 若不成立的影响 | 是否已向用户确认 |
|--------|---------|------|--------------|----------------|
| ASM-001 | [假设内容] | [推断依据] | [后果] | Y/N |

## 3. 继续生成的前提条件

- [列出所有"若要继续推进，必须先满足"的条件]
- [每一项条件对应哪个阻断项或假设项]
```
...
| Q0-4 | 编码正确 | 可正确读取无乱码 |
| Q0-5 | 质量报告已生成 | 至少输出 `00_input_validation_report.md` 与 `00_normalized_input.md`；若存在缺口或阻断，必须同时输出 `00_enhancement_suggestions.md` / `00_blockers_and_assumptions.md` |
