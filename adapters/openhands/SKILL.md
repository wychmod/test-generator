---
name: testcase-generator-openhands-adapter
description: 适配 OpenHands 的入口文件。Use when the host is OpenHands and expects a SKILL.md-style entry under its skills directory, and needs an entry for test case generation, requirement review, code-assisted testing, or structured testcase outputs.
---

# OpenHands Adapter for Testcase Generator

这是 `testcase-generator` 在 **OpenHands** 中的适配入口。

## 适配依据

OpenHands 官方技能目录为项目级 `.openhands/skills/<name>/SKILL.md`、用户级 `~/.openhands/skills/<name>/SKILL.md`，因此本技能**无需格式转换**即可被发现 ——
...
- OpenHands 具备沙箱执行能力，可在 Phase 2 直接用真实运行结果校正代码分析结论。
...
OpenHands **不保证** Python 运行时可用。若脚本无法执行：
