---
name: testcase-generator-mcpjam-adapter
description: 适配 MCPJam 的入口文件。Use when the host is MCPJam and expects a SKILL.md-style entry under its skills directory, and needs an entry for test case generation, requirement review, code-assisted testing, or structured testcase outputs.
---

# MCPJam Adapter for Testcase Generator

这是 `testcase-generator` 在 **MCPJam** 中的适配入口。

## 适配依据

MCPJam 官方技能目录为项目级 `.mcpjam/skills/<name>/SKILL.md`、用户级 `~/.mcpjam/skills/<name>/SKILL.md`，因此本技能**无需格式转换**即可被发现 ——
...
- MCPJam 侧重 MCP Server 调试，若被测对象是 MCP 工具，建议在 Phase 2 额外覆盖工具 schema 与错误码。
...
MCPJam **不保证** Python 运行时可用。若脚本无法执行：
