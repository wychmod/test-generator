# Cursor Rules — testcase-generator (soft adapter)

> This file is a **soft adapter** for Cursor (Anysphere). Cursor does not consume a `SKILL.md`-style skill entry; it consumes a single rules file at the project root: `.cursorrules`. The npm activation command in this repository will copy this file as `.cursorrules` so Cursor can pick it up automatically.
>
> Repository canonical source: the root `SKILL.md` and the six-phase pipeline in `prompts/`. This adapter does not redefine any of that — it only tells Cursor **when** to engage the Skill, **which order to read files in**, and **how to degrade** when context is tight.

## When to engage

Engage the testcase-generator workflow when the user asks Cursor to:

- generate test cases from a PRD, requirements doc, API spec, user story, or natural-language feature description
- review, expand, audit, or standardize existing test cases or test scenarios
- derive MBT-oriented test design (state machines, transition coverage, decision tables) for stateful workflows
- produce structured test design artifacts (testcase collections, traceability matrices, quality gate reports)
- analyze source code together with requirements to back-fill missing test paths or contract risks
...
If the user provides a local PRD / Markdown / PDF, prefer running `scripts/prd_reader.py` when the environment allows script execution.

## Working principles Cursor must respect

- Never invent requirements, business rules, or expected results. If you must infer, mark it explicitly as "推断 / inferred" or "假设 / assumed".
- Always preserve traceability between source (requirement / code) and conclusion (testcase).
...
Cursor is conversational, so context budget matters. When context is tight or the user wants a quick reply:
...
- Never claim a degraded draft is a "正式测试用例 / formal test case" — call it a draft.

If Cursor cannot read the full repository or cannot execute `scripts/prd_reader.py`:
...
- The npm `test-generator activate cursor` command copies this file as `.cursorrules` at the project root (or under the user's home directory with `-g`).
- The Chinese original of the methodology remains authoritative; English content here is for Cursor's English-first context.
- If the user also has a SKILL.md-style host (Claude / Qoder / CodeBuddy / Trae) running in the same repo, prefer the canonical `SKILL.md` for that host — `.cursorrules` only applies when Cursor is the active tool.
