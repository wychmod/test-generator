---
name: testcase-generator-antigravity-adapter
description: 适配 Google Antigravity 的入口文件。Use when the host is Google Antigravity and expects a SKILL.md-style entry under its skills directory, and needs an entry for test case generation, requirement review, code-assisted testing, or structured testcase outputs.
---

# Google Antigravity Adapter for Testcase Generator

这是 `testcase-generator` 在 **Google Antigravity** 中的适配入口。

## 适配依据

Antigravity 遵循 Agent Skills 标准布局，技能目录为项目级 `.agent/skills/<name>/SKILL.md`、用户级 `~/.gemini/antigravity/skills/<name>/SKILL.md`，因此本技能**无需格式转换**即可被发现 ——
...
- Antigravity 的 Agent 具备浏览器与终端能力，测试设计时应同时利用需求、实现与运行结果三类证据。
...
Google Antigravity **不保证** Python 运行时可用。若脚本无法执行：
