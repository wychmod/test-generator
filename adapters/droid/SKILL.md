---
name: testcase-generator-droid-adapter
description: 适配 Factory Droid 的入口文件。Use when the host is Factory Droid and expects a SKILL.md-style entry under its skills directory, and needs an entry for test case generation, requirement review, code-assisted testing, or structured testcase outputs.
---

# Factory Droid Adapter for Testcase Generator

这是 `testcase-generator` 在 **Factory Droid** 中的适配入口。

## 适配依据

Factory Droid 官方技能目录为项目级 `.factory/skills/<name>/SKILL.md`、用户级 `~/.factory/skills/<name>/SKILL.md`；并兼容 `.agent/skills/`，因此本技能**无需格式转换**即可被发现 ——
...
- Droid 面向「委派式」长任务，建议一次给出完整目标（被测范围 + 期望产物），由 Droid 自行分阶段推进。
...
Factory Droid **不保证** Python 运行时可用。若脚本无法执行：
