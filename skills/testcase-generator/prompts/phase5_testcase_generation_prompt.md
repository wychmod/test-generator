...
如果输入中包含 `[参考知识]`，先完成知识库引用整理，再生成测试用例。

处理规则：

- 当前需求、Phase 1-4 产物优先；知识库不能覆盖用户当前需求。
- 知识库规则只用于补充术语、测试数据候选、边界候选、历史易漏点和回归风险。
- 使用知识库内容生成或调整用例时，必须在“追溯引用”中写 `KB:` 来源。
- 如果知识库与当前需求冲突，不要静默采用知识库；在用例说明或风险摘要中标注 `KB 冲突`。
- 历史用例不能原样复制为新用例，必须结合当前需求重写步骤和预期结果。
...
| 用例ID | 场景标题 | 追溯引用 |
|---|---|---|
| TC-AUTH-PASSWORD-001 | 密码少于 8 位时提交失败 | REQ-AUTH-001; KB: project-conventions.md#密码强度规则 |
```
...
| # | 检查项 | 标准 | 不通过的处理 |
|---|-------|------|-------------|
| SC5-1 | 需求覆盖完整 | 100% 的可测试需求至少被一个用例覆盖 | 补充遗漏需求的用例 |
| SC5-2 | 边界覆盖完整 | Phase 1 中所有 BV 和 EP 都有对应用例 | 补充缺失的边界用例 |
| SC5-3 | 路径覆盖达标 | Phase 4 最小测试集中的所有路径都有用例 | 补充缺失路径的用例 |
| SC5-4 | 用例可执行 | 每条用例的前置条件和步骤充分 | 标记 [需补充] 并补充细节 |
| SC5-5 | 预期结果明确 | 每条用例的预期结果是二元判定的 | 重写模糊的预期结果 |
| SC5-6 | ID 唯一无重复 | 所有 TC-ID 全局唯一 | 重新编号 |
| SC5-7 | 追溯矩阵闭合 | 正向和逆向追溯均可查询 | 补充断裂的追溯链接 |

### 警告项

| # | 触发条件 | 处理 |
|---|---------|------|
| SW5-1 | 总用例数 > 200 | 评估是否需要分批管理 |
| SW5-2 | P0 用例 > 30% | 重新评估优先级分配是否合理 |
| SW5-3 | 自动化可行但未标记 | 建议增加自动化标注 |
| SW5-4 | 存大量单点覆盖 | 建议增加关键路径的冗余覆盖 |
