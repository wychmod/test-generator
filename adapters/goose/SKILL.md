---
name: testcase-generator-goose-adapter
description: 适配 Goose（Block） 的入口文件。Use when the host is Goose（Block） and expects a SKILL.md-style entry under its skills directory, and needs an entry for test case generation, requirement review, code-assisted testing, or structured testcase outputs.
---

# Goose（Block） Adapter for Testcase Generator

这是 `testcase-generator` 在 **Goose（Block）** 中的适配入口。

## 适配依据

Goose 官方技能目录为项目级 `.goose/skills/<name>/SKILL.md`、用户级 `~/.config/goose/skills/<name>/SKILL.md`，因此本技能**无需格式转换**即可被发现 ——
...
- Goose 可调用扩展（MCP）执行命令，若可用则可直接运行 `scripts/prd_reader.py` 解析本地 PRD。
...
Goose（Block） **不保证** Python 运行时可用。若脚本无法执行：
