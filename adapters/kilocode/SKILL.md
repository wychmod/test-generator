---
name: testcase-generator-kilocode-adapter
description: 适配 Kilo Code 的入口文件。Use when the host is Kilo Code and expects a SKILL.md-style entry under its skills directory, and needs an entry for test case generation, requirement review, code-assisted testing, or structured testcase outputs.
---

# Kilo Code Adapter for Testcase Generator

这是 `testcase-generator` 在 **Kilo Code** 中的适配入口。

## 适配依据

Kilo Code 官方技能目录为项目级 `.kilocode/skills/<name>/SKILL.md`，因此本技能**无需格式转换**即可被发现 ——
...
- Kilo Code 支持多模式切换，测试设计建议使用其 Architect / Code 模式以获得代码级上下文。
...
Kilo Code **不保证** Python 运行时可用。若脚本无法执行：
