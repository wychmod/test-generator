---
name: testcase-generator-amp-adapter
description: 适配 Amp（Sourcegraph） 的入口文件。Use when the host is Amp（Sourcegraph） and expects a SKILL.md-style entry under its skills directory, and needs an entry for test case generation, requirement review, code-assisted testing, or structured testcase outputs.
---

# Amp（Sourcegraph） Adapter for Testcase Generator

这是 `testcase-generator` 在 **Amp（Sourcegraph）** 中的适配入口。

## 适配依据

Amp 遵循 Agent Skills 标准布局，技能目录为项目级 `.agents/skills/<name>/SKILL.md`、用户级 `~/.config/agents/skills/<name>/SKILL.md`，因此本技能**无需格式转换**即可被发现 ——
...
- Amp 强调「按需加载上下文」，建议先只读技能根 `SKILL.md`，再按任务规模决定是否展开 `prompts/`。
...
Amp（Sourcegraph） **不保证** Python 运行时可用。若脚本无法执行：
