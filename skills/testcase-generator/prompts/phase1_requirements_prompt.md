...
> **版本**: 2.3.0 | **阶段目标**: 将任意格式的原始输入转化为结构化、可测试、可追溯的需求规格
...
- ISO/IEC/IEEE 29119-3:2021 / INCOSE 需求规格标准实践者
...
如果 Phase 0 或用户输入提供了 `[参考知识]`，必须把它作为知识库辅助上下文处理。

使用方式：

- 用 `KB: <filename>#<heading>` 来源统一术语、状态枚举、错误码、字段约定和历史经验。
- 当前需求优先于知识库。知识库不能覆盖用户当前需求；出现差异时标注 `KB 冲突`，并说明冲突双方。
- 从知识库补充的边界、异常、权限、状态流转只能标为“候选”或“参考”，除非当前需求也明确支持。
- 需求、业务规则或验收标准引用知识库时，在“来源追溯”字段追加 `KB:` 引用。
- 不得把历史用例直接当成需求；历史用例只能帮助发现易漏场景和回归风险。
...
| KB 来源 | 被用于 | 处理结论 |
|---|---|---|
| KB: project-conventions.md#密码强度规则 | 术语/规则统一 | 用作密码字段边界候选 |
| KB: historical-cases.md#TC-AUTH-LOGIN-007 | 历史回归参考 | 提醒覆盖锁定计数器重置 |
```
...
| `04_nfr_and_impact_analysis.md` | 非功能需求 + 影响映射 + 用户旅程（v2.1，见 §8-§10） | 输入含 NFR 或存在跨模块影响 |
...
| # | 检查项 | 检查方法 | 通过标准 |
|---|-------|---------|---------|
| SC1-1 | 需求完整性 | 对照原始输入逐项核对 | 所有功能点都已提取，无遗漏 |
| SC1-2 | 需求可测试性 | 检查每条 REQ 的 AC | 每条 REQ 至少有 1 条可二元判定的 AC |
| SC1-3 | 边界覆盖完整 | 检查参数分析表 | 所有输入/输出参数都有 BVA 和 EP |
| SC1-4 | 歧义已标注 | 搜索未处理的模糊表述 | 模糊表述均已标记 [需确认] 或 [推断] |
| SC1-5 | ID 唯一性 | 检查所有 ID | REQ/AC/BR/EQ/PARAM ID 无重复 |
| SC1-6 | 术语一致性 | 全局搜索关键术语 | 同一概念全程使用相同术语 |

#### 警告项（不阻塞但需记录）：

| # | 检查项 | 触发条件 | 处理方式 |
|---|-------|---------|---------|
| SW1-1 | 需求数量异常 | REQ < 3 或 > 50 | 记录到质量报告，评估是否需要进一步拆分/合并 |
| SW1-2 | 高比例歧义 | [需确认] 标记 > 30% | 建议用户提供更详细的需求文档 |
| SW1-3 | 冲突未解决 | conflict_count > 0 | 在报告中列出所有冲突，提醒后续阶段注意 |
| SW1-4 | AC 质量不足 | 含主观词的 AC > 20% | 尝试改写，记录原始版本 |
...
>
> **产物归属**：本节内容写入 `04_nfr_and_impact_analysis.md`（Part A）。
...
>
> **产物归属**：本节内容写入 `04_nfr_and_impact_analysis.md`（Part B）。
...
>
> **产物归属**：本节内容写入 `04_nfr_and_impact_analysis.md`（Part C）。
