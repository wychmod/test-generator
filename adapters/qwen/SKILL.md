---
name: testcase-generator-qwen-adapter
description: 适配 Qwen Code 的入口文件。Use when the host is Qwen Code and expects a SKILL.md-style entry under its skills directory, and needs an entry for test case generation, requirement review, code-assisted testing, or structured testcase outputs.
---

# Qwen Code Adapter for Testcase Generator

这是 `testcase-generator` 在 **Qwen Code** 中的适配入口。

## 适配依据

Qwen Code 官方技能目录为项目级 `.qwen/skills/<name>/SKILL.md`、用户级 `~/.qwen/skills/<name>/SKILL.md`，因此本技能**无需格式转换**即可被发现 ——
...
- Qwen Code 对中文需求文档理解较好，可直接投喂中文 PRD，无需预先翻译。
...
Qwen Code **不保证** Python 运行时可用。若脚本无法执行：
