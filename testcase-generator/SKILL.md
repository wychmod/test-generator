...
# 🔬 TestCase Generator v2.0 — 生产级测试用例生成器
...
| 阶段 | 名称 | 关键产出 | 文件数 | 依赖阶段 |
|------|------|---------|--------|---------|
| **Phase 1** | 需求预处理 | 结构化需求 + 边界条件 + 可追溯矩阵 | 3 | 无 |
| **Phase 2** | 代码分析 | 代码结构 + 数据流 + 缺陷图谱 | 3 | P1 |
| **Phase 3** | 领域建模 | 业务实体 + 状态机 + 测试参数空间 | 3 | P1, P2 |
| **Phase 4** | MBT设计 | 测试模型规格 + 状态转换图 + 覆盖准则 | 3 | P1, P2, P3 |
| **Phase 5** | 用例生成 | 测试用例集 + 执行套件 + 覆盖矩阵 | 3 | P1-P4 |
...
phase4/
  ├── 01_test_model_specification.md   # 测试模型完整规格
  ├── 02_state_transition_graph.md     # 状态转换图 (Mermaid)
  └── 03_coverage_criteria.md          # 覆盖准则与路径策略

phase5/
  ├── 01_testcase_collection.md        # 完整测试用例集
  ├── 02_test_suite_summary.md         # 测试套件摘要统计
