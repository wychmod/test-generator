---
name: testcase-generator-cline-adapter
description: 适配 Cline 的入口文件。Use when the host is Cline and expects a SKILL.md-style entry under its skills directory, and needs an entry for test case generation, requirement review, code-assisted testing, or structured testcase outputs.
---

# Cline Adapter for Testcase Generator

这是 `testcase-generator` 在 **Cline** 中的适配入口。

## 适配依据

Cline（3.48.0 起）支持技能，目录为项目级 `.cline/skills/<name>/SKILL.md`、用户级 `~/.cline/skills/<name>/SKILL.md`；同时兼容 `.clinerules/skills/` 与 `.claude/skills/`，因此本技能**无需格式转换**即可被发现 ——
...
- Cline 以「人工逐步确认」为协作模型，建议在生成测试用例前先给出测试点清单供用户确认。
...
Cline **不保证** Python 运行时可用。若脚本无法执行：
