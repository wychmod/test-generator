...
### 已闭环的缺口：`docs/` 曾不受任何审计覆盖

P1-4 的路径迁移曾遗漏 `docs/` 下的失效引用，根因是 **`docs/` 不受任何一致性审计覆盖**，
因此机器完全没有发现。该缺口已在 v2.3.0 内闭环：

| 新增审计项 | 作用 |
|---|---|
| `docs_skill_tree_paths` | docs/ 引用技能内容时必须带 `skills/testcase-generator/` 前缀 |
| `docs_markdown_links` | docs/ 的相对链接必须能解析到真实文件 |

两项检查的判定都是**零猜测**的：

- 路径项只在"该文件在技能树下存在、且在仓库根不存在"时才判失效 —— 因此
  **不会误报**用裸目录名描述**其他项目**布局的段落（Agent Skills 标准本身就含
  `scripts/` / `references/` / `assets/`），也不会误报描述迁移前后差异的历史叙述。
- 链接项只检查 `](url)` 形式，因此正文里把 `SKILL.md` 当概念提到的场合不受影响。

修正范围：90+ 处技能树路径引用（含通配 / 占位 / 省略号形式）、4 个指向
`../../SKILL.md` 的死链、以及若干陈旧版本号示例（外链中的
`oasis-open.org/.../v2.1.0` 与 Claude Code 自身的 `v2.1.277` 按规则保持原样）。
两项检查均已并入 CI，同类遗漏不会再静默发生。
