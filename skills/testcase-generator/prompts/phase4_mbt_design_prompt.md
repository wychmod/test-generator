...
| # | 检查项 | 标准 |
|---|-------|------|
| SC4-1 | 模型可追溯 | 每个测试对象/转换都可追溯到 Phase 3 的实体/状态 |
| SC4-2 | 状态图一致性 | 与 Phase 3 状态机一致（允许合理的合并/抽象） |
| SC4-3 | 覆盖准则合理 | 符合 ISTQB 标准，且用例数量在合理范围内 |
| SC4-4 | 路径完整性 | 列出的路径覆盖了初始到终态的所有合法路径 |
| SC4-5 | 无冗余路径 | 已识别并标注了可以合并的等价路径 |

### 警告项

| # | 触发条件 | 建议 |
|---|---------|------|
| SW4-1 | 最小测试集 > 50 条 | 评估是否需要提高抽象层级减少路径 |
| SW4-2 | 单点覆盖元素 > 30% | 建议增加冗余覆盖提高鲁棒性 |
| SW4-3 | 应用了大量模型简化 | 记录所有简化决策供审核 |
