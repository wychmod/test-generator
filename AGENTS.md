...
`testcase-generator` 是一个 **Agent Skill 包**（不是普通库）：仓库根目录就是 canonical source，`SKILL.md` 是中文主入口，`skill.manifest.json` 同时承担分发边界与版本契约。它为 8 个 AI 宿主提供触发式测试用例生成能力。
...
# 改过 SKILL.md / prompts / resources / templates / references 之后
python devtools/capability_audit.py
python devtools/skill_quality_audit.py

# 改过 SKILL.md / README / manifest / adapters / HOST_COMPATIBILITY 之后
...
- **版本的唯一数据源是 `skill.manifest.json` 的"版本"字段。** 不要手改 `SKILL.md` / `README.md` / `prompts/` / `templates/` 里的版本号，跑 `sync_version.py --write` 回写。
...
- **`adapters/<host>/` 是薄适配层**，只做入口文件名、触发词、资源路由、降级说明四件事；不得复制 `prompts/` `templates/` `resources/` 的实际内容，不得重新定义方法论。
...
- **核心能力固定 7 项**，改动必须同时更新 `SKILL.md` / `prompts/phase*.md` / `resources/output_artifacts.md` / `skill.manifest.json` 四处，否则跑 `doc_consistency_audit.py` 会失败。

---

## 边界

- 以下目录**全部已 gitignore**，由 `test-generator activate` 生成：`.claude/` `.codebuddy/` `.agents/` `.cursor/` `.qoder/` `.trae/` `.windsurf/` `skills/` `node_modules/`。不要手工编辑，不要强制提交。
- `.skill` / `.zip` 是打包产物（包中包），不要提交。
- `test-output/` 是本地验证产物，不要提交。
- **不要删除 `.workbuddy/`** —— 里面是项目记忆与工作日志。
- 不要手工编辑 `knowledge/index.json`（由 `build_index.py` 生成，且不入包）。

---

## 坑（已经踩过的）

- **openclaw 的入口名 `skill.md` 与 canonical `SKILL.md` 在 Windows / macOS 上是同一个文件。** 激活时检测到该冲突会**跳过写入**并保留 canonical，同时输出 `skipped 'skill.md'` 提示——这是预期保护行为，不是 bug。
...
- **`capability_audit.py` 的部分能力标记扫描的是 `resources/output_artifacts.md`**，不是 `SKILL.md`。把 SKILL.md 的章节下沉到 `references/` 时，请先确认标记仍可被审计发现。
